...
@section('page.title', 'اضافه صور للجدول'.' : '.$table->title)

@section('content')
    <div class="card">
        <section class="section main-block">
            <h1 class="title"><a href="{{ route('admin.tables.index') }}" class="button">
                    <span class="icon is-small"><i class="fa fa-newspaper"></i></span>
                    <span>قائمة الجداول</span>
                </a></h1>
            <dropzone
                url="{{ route('admin.store_photo', $table->id) }}"
                redirect-link="{{ route('admin.tables.show', $table->id) }}"
                delete-url="#"
            ></dropzone>
